...
function Choice({
  ...
  return (
    <div
      className={` w-96 border-4 box-border flex flex-row items-center gap-3 p-2 rounded-lg mt-2 ${value}`}
    >
      <label
        htmlFor={`choice-${question._id}-${choice}`}
        className={`${!submitted ? "cursor-pointer" : ""} flex flex-row items-center gap-3 w-full h-full p-2}`} 
      >
        <input
          type="radio"
        ...
        />
        <span className="text-base flex justify-between w-full"><p>{question[choice]}</p> <p>percent</p></span>
      </label>
    </div>
  );
}
