...
function Buttons() {
  ...
  return (
    <motion.div
      ...
      animate="final"
      onAnimationComplete={() => {
        localStorage.setItem("animation", previousAnimation === "-100vh" ? "100vh" : "-100vh");
      }}
    >
      <div className="flex flex-col justify-center items-center text-2xl text-[#FFFFFF] gap-20 bg-black p-20 rounded-3xl border-[#302f2f] border">
        ...
        <div className="flex flex-col gap-4 items-center">
          <Link className="visited:text-black flex justify-center rounded-lg px-3 py-5 border text-xl-1  bg-white w-60 font-medium text-black" to="/user">
            {" "}
            Login as User{" "}
          </Link>
          <Link className="visited:text-black flex justify-center rounded-lg px-3 py-5 border text-xl-1  bg-white w-60 font-medium text-black" to="/admin">
            Login as Admin
          </Link>
        </div>
      </div>
